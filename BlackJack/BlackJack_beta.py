...
try:
    ...
    def deal_hand():
        ...
        sleep(1)
        print(
            f"Your hand contains '{str(player_hand2[0])}' and '{str(player_hand2[1])}'.\n"
        )
        hand_value()
    ...
    def hit_stay():
        global first_draw
        if first_draw == 0:
            print(f"The dealer draws two cards and reveals '{str(dealer_hand2[0])}'.\n")
            first_draw += 1
        if player_hand_value <= 21:
            hit_stay_choice = ""
    ...
    def reveal_dealer_hand():
        ...
        if dealer_hand_value > 21:
            if "Ace" in dealer_hand[0]:
                dealer_hand_value -= 10
        print(
            f"\nThe dealer's hand is {str(dealer_hand2)} with a value of {dealer_hand_value}. "
        )
    ...
    def dealer_hit():
        ...
        dealer_hand2.append(str(dealer_hand[0][-1]) + " of " + random.choice(suits))
    ...
    def win_game():
        print(
            f"\nYour hand value was {player_hand_value} while the dealer's hand value was {dealer_hand_value}."
        )
        sleep(1)
        if dealer_hand_value > 21:
    ...
    def ask_game():
        ...
        if new_game == "Yes" or new_game == "yes" or new_game == "y" or new_game == "Y":
            ...
            deal_hand()
        elif new_game == "No" or new_game == "no" or new_game == "n" or new_game == "N":
    ...
    print(
     ...
     Objective: To have a higher score(hand value) than the dealer but at the same time to not go over 21.
    
     1. You get a pre-dealt pair of cards from the dealer. These two cards' value will be calculated as your score.
     2. The values of the cards King, Queen, Jack have 10 points each and the rest of the cards (2-10) have their
         unit values.
     3. "Hit" means to draw another card (the value will be added to your hand) and if you feel your hand value is
         capable of having another card, you may "hit" to get another card
     4. "Stand" means that you are currently not drawing any card and thus the hand value will not change and if you are
         satisfied with your hand, you may "stand" with it.
     5. If anyone's hand value goes over 21, they automatically lose or "bust".
     6. The Ace card value can change between 11 or 1 depending on the overall value of the hand.
